...
module {
    ...
    public func recordTransaction(
        ...
        let existingTransactions = switch (transactionHistory.get(user)) {
            case (?transactions) { transactions };
            case null { [] };
        };
        
        transactionHistory.put(user, Array.append(existingTransactions, [transactionId]));
    };
    ...
    public func getReputationStats(
        ...
        let userTransactions = switch (transactionHistory.get(user)) {
            case (?transactions) { transactions };
            case null { [] };
        };
        ...
        
        {
            ...
            totalTransactions = Array.size(userTransactions);
            successfulTransactions = 0; // Would need to track this separately
